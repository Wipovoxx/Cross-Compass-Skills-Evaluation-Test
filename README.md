...
### Display modes
Three display modes are available: **Side by side** shows the original and preview simultaneously, **Original Only** and **Preview Only** show a single enlarged image for closer inspection.
...
│       └── constants.py      # Shared constants (extensions, default values, warning message)
├── tests/
│   ├── test_edit_image.py    # Unit tests for image editing logic
│   └── test_scan_images.py   # Unit tests for image scanning logic
├── source_images/            # Sample images to try the app with
├── examples/                 # Reference examples from the pyedifice documentation
├── pyproject.toml
└── uv.lock
```
...
- **`DisplayModeWidget`** — toggle buttons for switching between side-by-side, original-only, and preview-only display modes.
- **`OnboardingWidget`** — welcome screen shown until both folders are selected, with dynamic checkmarks as each step is completed.
- **`ImageComponent`** — displays either the original or the live-edited preview image, resizing based on the active display mode.
